...
func NewBoshComplete(blindUi *boshui.ConfUI, logger boshlog.Logger) *BoshComplete {
	cmdFactory := boshcmd.NewFactory(boshcmd.NewBasicDeps(blindUi, logger))
	var session boshcmd.Session
	cmd, err := cmdFactory.New([]string{cobraCompletionCmdName}) // just to init session
	if err != nil {
		logger.Debug(logTag, "session initialization (command '%s') error: %v", cobraCompletionCmdName, err)
	} else {
		session = cmd.ExportSession()
	}
	cmdContext := &CmdContext{}
