...
func (c *DirectorQuery) cache(group string) *CompleteCache {
	cacheDir := filepath.Join(filepath.Dir(c.cmdContext.ConfigPath), "completion-cache")
	cacheKey := group
	if c.cmdContext.DeploymentName != "" {
		group = group + "." + c.cmdContext.DeploymentName
	}
	c.logger.Debug(c.logTag, "cache directory '%s', cache key '%s', command context: %v", cacheDir, cacheKey, c.cmdContext)
	return NewCompleteCache(c.logger, cacheDir, cacheKey)
}
...
func (c *DirectorQuery) listDeploymentNames(prefix string) (names []string, err error) {
	cache := c.cache("deployments")
	if values, valid, _ := cache.GetValues(); valid {
		c.logger.Debug(c.logTag, "Deployments read from cache")
	...
	_ = cache.PutValues(newValues)
	return FilterQueryValues(newValues, prefix), nil
}

func (c *DirectorQuery) getCacheOrDirector(group string, prefix string) (names []string, validCache bool, cache *CompleteCache, director boshdir.Director, err error) {
	cache = c.cache(group)
	if values, valid, _ := cache.GetValues(); valid {
		c.logger.Debug(c.logTag, "Group %s read from cache", group)
		return FilterQueryValues(values, prefix), true, cache, nil, nil
	}
	...
	return []string{}, false, cache, director, err
}
